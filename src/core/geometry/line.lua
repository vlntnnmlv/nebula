...
function Line:init(scene, root, a, b)
    ...
    local vertices = {}
    table.insert(vertices, { 0.01, 0.0 })
    table.insert(vertices, { 1.0, 0.99 })
    table.insert(vertices, { 0.0, 0.01 })
    table.insert(vertices, { 0.99, 1.0 })

    self.mesh = love.graphics.newMesh(vertices, "strip")
end

function Line:drawInternal()
    love.graphics.draw(self.mesh, 0, 0, 0, self.w, self.h)
end